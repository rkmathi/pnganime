...
def create_diff(fst, end, cd, digit):
    for cnt in range(fst, end):
        ...
        #diff.convert('RGBA').save("LL2-diff/%06d_.png" % (cnt))
        diff.convert('RGBA').save("LL22-diff/%06d_.png" % (cnt))
...
def generate_diff_img(img1, img2):
    ...
    for x in range(_img1.size[0]):
        for y in range(_img1.size[1]):
            if (abs(__img1[x, y][0] - __img2[x, y][0]) >= Q or
                abs(__img1[x, y][1] - __img2[x, y][1]) >= Q or
                abs(__img1[x, y][2] - __img2[x, y][2]) >= Q):
...
if __name__ == '__main__':
    #create_diff(581, 682, "miku2/", 8)
    create_diff(0, 46, "miku1/", 8)
