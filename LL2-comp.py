...
def create_diff(img1, img2, cd1, cd2, suffix):
    ...
    ans.convert('RGBA').save("LL2-comp/%s-%s.png" % (img1, img2))
...
def create_all(fst, end, cd1, cd2, digit):
    ...
    for i in range(fst, end):
        ...
        create_diff(i, i+1, directory1, directory2, "_.png")
        print("%d end!" % i)


if __name__ == '__main__':
    create_all(581, 680, "LL2-diff/", "LL2-diff/", 3)
    
    """
    key  = Image.open("miku2/00000582.png")
    back = Image.open("2-c1-diff/000581_.png")
    fore = Image.open("2-c1-comp/581-582.png")
    key.paste(back, (0, 0), back)
    key.paste(fore, (0, 0), fore)
    #key.show()
    key.convert('RGBA').save("hoge2.png")
    """